...
PHONELAB_DOC_PATTERN = re.compile(\
    r"""^\s*/\*\*\s*\n\s*\*\s*PhoneLab.*?(?P<json>\{.*?\})\s*\n\s*\*\/\s*\n""",
    re.VERBOSE | re.DOTALL | re.MULTILINE)


DEFAULT_AOSP_BASE = '5.1.1_r3'
...
class RepoProject(object):
  ...
  @property
  def url(self):
    return '%s/gitweb?p=%s.git' % (HTTP_SERVER, self.name)
  ...
  def get_file_url(self, file_path, line_no=None):
    url = '%s/gitweb?p=%s.git;a=blob;f=%s;hb=refs/heads/%s' % (HTTP_SERVER,\
        self.name, self.get_relative_path(file_path), self.current_branch)
...
class TagDoc(object):
  ...
  def create_from_file(cls, proj, src_file):
    ...
    with open(src_file, 'r') as f:
      s = f.read()
      for match in PHONELAB_DOC_PATTERN.finditer(s):
        try:
          doc = json.loads(match.group('json').replace('*', ''))
          tag_docs.append(TagDoc(doc, proj, src_file, s.count('\n', 0,\
              match.start())+1))
        except:
          logger.exception("Invalid doc string in file %s: %s" % (src_file,\
              match.group('json')))
  ...
  def create_from_proj(cls, proj):
    ...
    for dirpath, dirnames, filenames in os.walk(proj.abs_path):
      for f in filenames:
        tag_docs.extend(cls.create_from_file(proj, os.path.join(dirpath, f)))
...
class HTMLFormatter(object):
  ...
  def __repr__(self):
    ...
    for category in sorted(list(set([t.category for t in self.tag_docs]))):
      ...
      category_tags = [t for t in self.tag_docs if t.category == category]
...
def main():
  ...
  release_branch_prefix = 'phonelab/android-%s/release-' % (args.aosp)
  develop_branch = 'phonelab/android-%s/develop' % (args.aosp)
  ...
  if len(all_branches) > 1:
    logger.error("Not all projects in same branch, do a `repo status` and check.")
    # return
